...
const SEARCH_URLS = [
  'https://www.aplikuj.pl/praca/informatyka-it',
  'https://www.aplikuj.pl/praca/informatyka-it?kw=react',
  'https://www.aplikuj.pl/praca/informatyka-it?kw=node',
  'https://www.aplikuj.pl/praca/informatyka-it?kw=python',
  'https://www.aplikuj.pl/praca/informatyka-it?kw=java',
  'https://www.aplikuj.pl/praca/informatyka-it?kw=devops',
  'https://www.aplikuj.pl/praca/informatyka-it?kw=frontend',
  'https://www.aplikuj.pl/praca/informatyka-it?kw=backend',
  'https://www.aplikuj.pl/praca/informatyka-it?kw=fullstack',
];

export const aplikujScraper: Scraper = {
  name: 'aplikuj',
  ...
  async fetchJobs(): Promise<JobOfferInput[]> {
    ...
    for (const url of SEARCH_URLS) {
      try {
        ...
        const jobs = parseAplikujHtml(html);
        ...
        console.warn(`aplikuj: query "${url}" failed -`, e instanceof Error ? e.message : e);
      }
    }
...
function parseAplikujHtml(html: string): JobOfferInput[] {
  const jobs: JobOfferInput[] = [];

  const cardRegex = /<a[^>]*href="(\/oferta\/[^"]+)"[^>]*>[\s\S]*?<\/a>/gi;
  let match;
  ...
  while ((match = cardRegex.exec(html)) !== null) {
    try {
      ...
      const salaryMatch = cardHtml.match(/(\d[\d\s,.]*)\s*(?:-|–)\s*(\d[\d\s,.]*)\s*(?:zł|PLN|netto|brutto)/i)
      ...
      const sourceUrl = `https://www.aplikuj.pl${href}`;
